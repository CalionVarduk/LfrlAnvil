...
internal sealed class AlternativeAssertion : Assertion
{
    ...
    public override void Go()
    {
        ...
        if ( errors.Count == 0 )
            Throw( $"{(Context.Length > 0 ? $"[{Context}]" : "ANY assertion")} failed to satisfy at least one assertion." );

        Throw(
            $"""
             {(Context.Length > 0 ? $"[{Context}]" : "ANY assertion")} failed to satisfy at least one assertion, with following {errors.Count} error(s):
