...
using AssertionScope = FluentAssertions.Execution.AssertionScope;

namespace LfrlAnvil.Chrono.Tests.CachingTests;

public class LifetimeCacheTests : TestsBase
{
    [Theory]
    [InlineData( 1, 1 )]
    [InlineData( 2, 100 )]
    ...
    public void Ctor_ShouldCreateEmptyWithCorrectLifetimeAndCapacity(int capacity, int lifetimeTicks)
    {
        ...
        var sut = new LifetimeCache<string, int>( timestamps, lifetime, capacity );
    ...
    public void Ctor_ShouldCreateEmptyWithCorrectLifetimeAndCapacity_WithExplicitComparer(int capacity, int lifetimeTicks)
    {
        ...
        var timestamps = new Timestamps();
        var lifetime = Duration.FromTicks( lifetimeTicks );
        var sut = new LifetimeCache<string, int>( comparer, timestamps, lifetime, capacity );
    ...
    public void Ctor_ShouldThrowArgumentOutOfRangeException_WhenCapacityIsLessThanOne(int capacity)
    {
        ...
        var action = Lambda.Of( () => new LifetimeCache<string, int>( timestamps, lifetime, capacity ) );
    ...
    public void Ctor_ShouldThrowArgumentOutOfRangeException_WhenLifetimeIsLessThanOneTick(int ticks)
    {
        ...
        var action = Lambda.Of( () => new LifetimeCache<string, int>( timestamps, lifetime, capacity: 1 ) );
    ...
    public void TryAdd_ShouldAddFirstItemCorrectly()
    {
        ...
        var sut = new LifetimeCache<string, int>( timestamps, lifetime: Duration.FromSeconds( 1 ), capacity: 3 );
    ...
    [Fact]
    public void Indexer_Getter_ShouldSetExistingEntryAsNewestAndUpdateLifetime_WhenKeyExistsAndIsNotNewest()
    {
        var timestamps = new Timestamps();
        var entry1 = KeyValuePair.Create( "foo", 1 );
        var entry2 = KeyValuePair.Create( "bar", 2 );
    ...
    [Fact]
    public void TryGetValue_ShouldSetExistingEntryAsNewestAndUpdateLifetime_WhenKeyExistsAndIsNotNewest()
    {
        var timestamps = new Timestamps();
        var entry1 = KeyValuePair.Create( "foo", 1 );
        var entry2 = KeyValuePair.Create( "bar", 2 );
    ...
    [Fact]
    public void Restart_ShouldSetExistingEntryAsNewestAndUpdateLifetime_WhenKeyExistsAndIsNotNewest()
    {
        var timestamps = new Timestamps();
        var entry1 = KeyValuePair.Create( "foo", 1 );
        var entry2 = KeyValuePair.Create( "bar", 2 );
