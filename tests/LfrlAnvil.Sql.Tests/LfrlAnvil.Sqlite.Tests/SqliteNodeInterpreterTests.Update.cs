...
WHERE EXISTS (
  ...
  GROUP BY ""f"".""b""
        ...
        public void Visit_ShouldInterpretUpdateComplexDataSource_WithComplexAssignmentAndSingleColumnPrimaryKey()
        {
            ...
            sut.Visit( dataSource.ToUpdate( s => new[] { s["f"]["b"].Assign( s["f"]["b"] + s["bar"]["b"] ) } ) );
...
WHERE ""common_foo"".""a"" IN (
  SELECT
    ...
    FROM ""_{GUID}""
        ...
        {
            var sut = CreateInterpreter();
            var table = SqlTableMock.Create<int>( "foo", new[] { "a", "b", "c" }, new[] { "a", "b" } );
            var foo = table.ToRecordSet( "f" );
            var dataSource = foo
                .Join( SqlJoinDefinition.Inner( SqlNode.RawRecordSet( "bar" ), x => x.Inner["a"] == foo["a"] ) )
                .GroupBy( s => new[] { s["f"]["b"] } );

            sut.Visit( dataSource.ToUpdate( s => new[] { s["f"]["c"].Assign( s["f"]["c"] + s["bar"]["c"] ) } ) );
