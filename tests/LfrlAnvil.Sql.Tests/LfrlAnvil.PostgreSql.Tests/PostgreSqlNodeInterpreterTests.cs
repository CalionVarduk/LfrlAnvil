...
public partial class PostgreSqlNodeInterpreterTests : TestsBase
{
    [Fact]
    ...
    [Fact]
    public void Visit_ShouldInterpretPositionalParameter()
    {
        var sut = CreateInterpreter();
...
""B"" AS (
  ...
  SELECT * FROM A WHERE A.depth < 10
    ...
    [InlineData( typeof( short ), "INT2" )]
    [InlineData( typeof( int ), "INT4" )]
    [InlineData( typeof( long ), "INT8" )]
...
COMMIT;" );
    ...
    [InlineData( IsolationLevel.Chaos, "REPEATABLE READ" )]
    ...
    public void Visit_ShouldInterpretBeginTransaction(IsolationLevel isolationLevel, string expectedSetSession)
    {
        var sut = CreateInterpreter();
    ...
    [Pure]
    private static PostgreSqlNodeInterpreter CreateInterpreter(PostgreSqlNodeInterpreterOptions? options = null)
    {
        return new PostgreSqlNodeInterpreter(
            options ?? PostgreSqlNodeInterpreterOptions.Default,
