...
public class PostgreSqlColumnBuilderTests : TestsBase
{
                ...
                actions.Select( a => a.Sql )
                    ...
                    [
                        (sql, _) => sql.TestSatisfySql(
                            ...
                            """
                            ALTER TABLE "foo"."T"
                                ADD COLUMN "C2" INT4 NOT NULL GENERATED ALWAYS AS IDENTITY,
                ...
                actions.Select( a => a.Sql )
                    ...
                    [
                        (sql, _) => sql.TestSatisfySql(
                            ...
                            """
                            ALTER TABLE "foo"."T"
                                ALTER COLUMN "C1" DROP IDENTITY,
                                ALTER COLUMN "C1" SET DATA TYPE INT8,
                                ADD COLUMN "C3" BYTEA,
                ...
                actions.Select( a => a.Sql )
                    ...
                    [
                        (sql, _) => sql.TestSatisfySql(
                            ...
                            """
                            ALTER TABLE "foo"."T"
                                ALTER COLUMN "C1" SET DATA TYPE INT4,
                    ...
                    [
                        (sql, _) => sql.TestSatisfySql(
                            ...
                            ALTER TABLE "foo"."T"
                                DROP CONSTRAINT "PK_T";
                            """,
                            "ALTER TABLE \"foo\".\"T\" RENAME COLUMN \"C1\" TO \"bar\";",
                            """
                            ALTER TABLE "foo"."T"
                                ADD CONSTRAINT "PK_T" PRIMARY KEY ("bar");
                            """ )
                    ] ) )
            .Go();
    }
    ...
    [Fact]
    public void SetIdentity_ShouldUpdateIdentity_WhenNewValueIsDifferentFromOldValueDueToCache()
    {
        var schema = PostgreSqlDatabaseBuilderMock.Create().Schemas.Create( "foo" );
        var table = schema.Objects.CreateTable( "T" );
        var sut = table.Columns.Create( "C2" ).SetType<int>().SetIdentity( SqlColumnIdentity.Default );
    ...
    [Fact]
    public void SetIdentity_ShouldUpdateIdentity_WhenNewValueIsDifferentFromOldValueDueToCache_WithDefaultCache()
    {
        var schema = PostgreSqlDatabaseBuilderMock.Create().Schemas.Create( "foo" );
        var table = schema.Objects.CreateTable( "T" );
        var sut = table.Columns.Create( "C2" ).SetType<int>().SetIdentity( new SqlColumnIdentity( 123 ) );
        table.Constraints.SetPrimaryKey( sut.Asc() );
        table.Columns.Create( "C1" );
    ...
    public void SetIdentity_ShouldBePossible_WhenColumnIsUsedInIndex()
    {
        ...
        var actionCount = schema.Database.GetPendingActionCount();
        var result = sut.SetIdentity( new SqlColumnIdentity( 1 ) );
        table.Constraints.SetPrimaryKey( result.Asc() );
        var actions = schema.Database.GetLastPendingActions( actionCount );
