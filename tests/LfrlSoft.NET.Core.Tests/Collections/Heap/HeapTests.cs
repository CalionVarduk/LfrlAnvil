...
{
    public abstract class HeapTests<T> : TestsBase
    {
        [Fact]
        ...
        private static void AssertHeapInvariant(Heap<T> heap)
        {
            ...
            for ( var parentIndex = 0; parentIndex <= maxParentIndex; ++parentIndex )
            {
                ...
                var leftChildIndex = Core.Collections.Heap.GetLeftChildIndex( parentIndex );
                var rightChildIndex = Core.Collections.Heap.GetRightChildIndex( parentIndex );
