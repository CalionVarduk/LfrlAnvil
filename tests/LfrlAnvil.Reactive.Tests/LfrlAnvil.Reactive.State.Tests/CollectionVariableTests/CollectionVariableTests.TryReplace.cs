...
{
    [Fact]
    public void TryReplace_ShouldReplaceExistingElementAndUpdateChangedFlag_WhenElementExistsAndNewValueIsNotEqualToOldValue()
    {
        ...
        var result = sut.TryReplace( element );
    ...
    [Fact]
    public void TryReplace_ShouldDoNothing_WhenElementExistsAndNewValueIsEqualToOldValue()
    {
    ...
    [Fact]
    public void TryReplace_ShouldUpdateErrorsAndWarnings_WhenElementIsReplaced()
    {
        var key = Fixture.Create<int>();
        var (oldValue, value) = Fixture.CreateManyDistinct<string>( count: 2 );
        var initialElement = new TestElement( key, oldValue );
    ...
    [Fact]
    public void TryReplace_ShouldUpdateNonInitialAddedElementCorrectly()
    {
        var key = Fixture.Create<int>();
        var (oldValue, value) = Fixture.CreateManyDistinct<string>( count: 2 );
    ...
    [Fact]
    public void TryReplace_ShouldDoNothing_WhenElementDoesNotExist()
    {
    ...
    [Fact]
    public void TryReplace_ShouldDoNothing_WhenStateContainsReadOnlyFlag()
    {
        var key = Fixture.Create<int>();
        var (oldValue, value) = Fixture.CreateManyDistinct<string>( count: 2 );
        var oldElement = new TestElement( key, oldValue );
    ...
    [Fact]
    public void
        TryReplace_WithRange_ShouldFilterElementsToReplaceCorrectlyAndReplaceOnlyThoseThatExistAndAreDifferentFromExistingElementAndDoNotRepeat()
    {
        ...
        var result = sut.TryReplace( elements );

        Assertion.All(
                result.TestEquals( VariableChangeResult.Changed ),
                sut.State.TestEquals( VariableState.Changed | VariableState.Dirty ),
                sut.Elements.Values.TestSetEqual(
                    [ allElements[0], allElements[1], elements[2], allElements[4], allElements[5], elements[8] ] ),
                ...
                onValidateEvents.TestCount( count => count.TestEquals( 1 ) )
                    .Then(
                        e => Assertion.All(
                            "validateEvent",
                            e[0].Elements.Select( el => el.Element ).TestSequence( [ elements[2], elements[8] ] ),
                            e[0].AssociatedChange.TestRefEquals( onChangeEvents.FirstOrDefault() ) ) ) )
            .Go();
    }

    [Fact]
    public void TryReplace_WithRange_ShouldReplaceSingleElementCorrectly_WhenItExists()
    {
        var key = Fixture.Create<int>();
        var (oldValue, value) = Fixture.CreateManyDistinct<string>( count: 2 );
        var initialElement = new TestElement( key, oldValue );
        ...
        var result = sut.TryReplace( new[] { element } );
    ...
    [Fact]
    public void TryReplace_WithRange_ShouldDoNothing_WhenAllElementsDoNotExist()
    {
        var element = Fixture.Create<TestElement>();
        var keySelector = Lambda.Of( (TestElement e) => e.Key );
        var sut = CollectionVariable.WithoutValidators<string>.Create( Array.Empty<TestElement>(), keySelector );
        ...
        var result = sut.TryReplace( new[] { element, element } );
    ...
    [Fact]
    public void TryReplace_WithRange_ShouldDoNothing_WhenElementsToReplaceAreEmpty()
    {
        ...
        var result = sut.TryReplace( Array.Empty<TestElement>() );
    ...
    [Fact]
    public void TryReplace_WithRange_ShouldDoNothing_WhenStateContainsReadOnlyFlag()
    {
        var key = Fixture.Create<int>();
        var (oldValue, value) = Fixture.CreateManyDistinct<string>( count: 2 );
        var initialElement = new TestElement( key, oldValue );
