...
public sealed class MathExpression<TArg, TResult> : IMathExpression<TArg, TResult>
{
    ...
    [Pure]
    public MathExpression<TArg, TResult> BindArguments(IEnumerable<KeyValuePair<string, TArg?>> arguments)
    {
        return BindArguments( arguments.Select( kv => KeyValuePair.Create( StringSlice.Create( kv.Key ), kv.Value ) ) );
    }

    [Pure]
    public MathExpression<TArg, TResult> BindArguments(params KeyValuePair<string, TArg?>[] arguments)
    {
        return BindArguments( arguments.AsEnumerable() );
    }

    [Pure]
    public MathExpression<TArg, TResult> BindArguments(IEnumerable<KeyValuePair<ReadOnlyMemory<char>, TArg?>> arguments)
    {
        return BindArguments( arguments.Select( kv => KeyValuePair.Create( StringSlice.Create( kv.Key ), kv.Value ) ) );
    }

    [Pure]
    public MathExpression<TArg, TResult> BindArguments(params KeyValuePair<ReadOnlyMemory<char>, TArg?>[] arguments)
    {
        return BindArguments( arguments.AsEnumerable() );
    }

    [Pure]
    public MathExpression<TArg, TResult> BindArguments(IEnumerable<KeyValuePair<int, TArg?>> arguments)
    {
        ...
        foreach ( var (index, value) in arguments )
        {
            if ( index < 0 || index >= _argumentIndexes.Count )
                throw new MathExpressionArgumentBindingException();
        ...
        return new MathExpression<TArg, TResult>( Input, expression, parameterBinder.ArgumentIndexes, parameterBinder.BoundArguments );
    }

    [Pure]
    public MathExpression<TArg, TResult> BindArguments(params KeyValuePair<int, TArg?>[] arguments)
    {
        return BindArguments( arguments.AsEnumerable() );
    }

    [Pure]
    public MathExpressionDelegate<TArg, TResult> Compile()
    {
        return new MathExpressionDelegate<TArg, TResult>( Expression.Compile(), _argumentIndexes );
    }
    ...
    [Pure]
    [MethodImpl( MethodImplOptions.AggressiveInlining )]
    private MathExpression<TArg, TResult> BindArguments(IEnumerable<KeyValuePair<StringSlice, TArg?>> arguments)
    {
    ...
    private sealed class ParameterBinder : ExpressionVisitor
    {
        ...
        public ParameterBinder(MathExpression<TArg, TResult> source, IReadOnlyDictionary<int, TArg?> argumentsToBind)
        {
    ...
    [Pure]
    IMathExpression<TArg, TResult> IMathExpression<TArg, TResult>.BindArguments(IEnumerable<KeyValuePair<string, TArg?>> arguments)
    {
        return BindArguments( arguments );
    }

    [Pure]
    IMathExpression<TArg, TResult> IMathExpression<TArg, TResult>.BindArguments(params KeyValuePair<string, TArg?>[] arguments)
    {
        return BindArguments( arguments );
    }

    [Pure]
    IMathExpression<TArg, TResult> IMathExpression<TArg, TResult>.BindArguments(
    ...
    [Pure]
    IMathExpression<TArg, TResult> IMathExpression<TArg, TResult>.BindArguments(IEnumerable<KeyValuePair<int, TArg?>> arguments)
    {
        return BindArguments( arguments );
    }

    [Pure]
    IMathExpression<TArg, TResult> IMathExpression<TArg, TResult>.BindArguments(params KeyValuePair<int, TArg?>[] arguments)
    {
        return BindArguments( arguments );
    }

    [Pure]
    IMathExpressionDelegate<TArg, TResult> IMathExpression<TArg, TResult>.Compile()
    {
        return Compile();
    }
}
