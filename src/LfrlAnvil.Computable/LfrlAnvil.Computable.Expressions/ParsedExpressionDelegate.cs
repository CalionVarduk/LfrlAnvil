...
public sealed class ParsedExpressionDelegate<TArg, TResult> : IParsedExpressionDelegate<TArg, TResult>
{
    ...
    internal ParsedExpressionDelegate(Func<TArg?[], TResult> @delegate, IReadOnlyDictionary<StringSlice, int> argumentIndexes)
    {
        Delegate = @delegate;
    ...
    public TResult Invoke(params TArg?[] arguments)
    {
        if ( _argumentIndexes.Count != arguments.Length )
            throw new InvalidParsedExpressionArgumentCountException( arguments.Length, _argumentIndexes.Count, nameof( arguments ) );
