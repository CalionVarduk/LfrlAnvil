...
{
    internal SqlConditionPlaceholderNode(string? identifier, bool wrapInParentheses)
        : base( SqlNodeType.ConditionPlaceholder )
    {
        Identifier = identifier ?? Guid.NewGuid().ToString( "N" );
        SqlPlaceholder = SqlHelpers.GetConditionSqlPlaceholder( Identifier );
