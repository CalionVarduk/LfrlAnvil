...
public sealed class TaskRegistry
{
    ...
    public TaskRegistry()
    {
        _taskContinuation = t => _tasks.TryRemove( t, out _ );
    }
    ...
    public void Add(Task task)
    {
        ...
        if ( ! _tasks.TryAdd( task, default ) )
            ExceptionThrower.Throw( new ArgumentException( ExceptionResources.TaskRegistryAlreadyContainsProvidedTask, nameof( task ) ) );
