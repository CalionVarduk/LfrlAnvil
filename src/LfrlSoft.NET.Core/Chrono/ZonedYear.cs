...
namespace LfrlSoft.NET.Core.Chrono
{
    public readonly struct ZonedYear : IEquatable<ZonedYear>, IComparable<ZonedYear>, IComparable
    {
        ...
        public int DayCount => IsLeap ? Constants.DaysInLeapYear : Constants.DaysInYear;
        public TimeZoneInfo TimeZone => Start.TimeZone;
        public Duration Duration => _duration ?? Duration.FromHours( Constants.HoursPerDay * Constants.DaysInYear );
        ...
        public IEnumerable<ZonedMonth> GetAllMonths()
        {
            ...
            for ( var month = 1; month <= Constants.MonthsPerYear; ++month )
        ...
        public IEnumerable<ZonedDay> GetAllDays()
        {
            ...
            for ( var day = 1; day <= Constants.DaysInJanuary; ++day )
                yield return ZonedDay.Create( new DateTime( year, (int)IsoMonthOfYear.January, day ), timeZone );

            var februaryDayCount = DateTime.IsLeapYear( year ) ? Constants.DaysInLeapFebruary : Constants.DaysInFebruary;
